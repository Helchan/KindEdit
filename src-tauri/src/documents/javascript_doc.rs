...
impl DocumentType for JavaScriptDocument {
    fn type_id(&self) -> &str {
        "javascript"
    }
    ...
    fn content_score(&self, content: &str) -> u8 {
        let keywords = ["function ", "const ", "let ", "var ", "import ", "export ", "=>", "require("];
...
mod tests {
    ...
    fn test_matches_path() {
        let doc = JavaScriptDocument;
        assert!(doc.matches_path("app.js"));
        assert!(doc.matches_path("module.mjs"));
        assert!(doc.matches_path("lib.cjs"));
        assert!(doc.matches_path("component.jsx"));
        assert!(doc.matches_path("types.ts"));
        assert!(doc.matches_path("page.tsx"));
        assert!(!doc.matches_path("file.py"));
    }
    ...
    fn test_content_score() {
        ...
        assert!(doc.content_score(js_code) > 50);
    ...
    fn test_parse_valid() {
        let doc = JavaScriptDocument;
        let code = "function test() { if (true) { return [1, 2]; } }";
