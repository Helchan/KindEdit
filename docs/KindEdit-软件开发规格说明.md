...
关于弹窗显示 `KindEdit v{version}` 和 `Rust + Tauri + React`。

应用图标：

- KindEdit 使用透明背景、后层蓝色文档页、前层白色文档页和绿色插入光标组成的简约图标，表达文本编辑、代码编辑和文档处理能力。
- 图标不显示外层直角底图或独立圆角底座；文档图形之外的区域必须保持透明，避免在桌面、Dock、启动器或安装包视图中出现黑色方块背景。
...
新建标签会追加到标签列表末尾并自动激活。关闭最后一个标签时，不会让应用进入无标签状态，而是替换为新的空白 `Untitled` 标签。

关闭脏标签时，应用通过系统对话框提示：

- 消息：`「{title}」有未保存的修改，确定要关闭吗？`
- 标题：`确认关闭`
- 类型：`warning`

用户取消时保持标签打开。对话框调用失败时，当前实现允许关闭。

关闭窗口时，如果存在未保存标签，应用通过系统对话框提示：

- 消息：`有未保存的修改，确定要关闭吗？`
- 标题：`确认关闭`
- 类型：`warning`

用户取消时保持窗口打开。确认后应用尝试保存会话，最多等待约 1.5 秒，然后销毁窗口。
...
- 标签关闭确认和窗口关闭确认在对话框调用失败时当前实现偏向继续关闭。后续若加强数据保护，应调整此策略并更新本文档。
